...
def build_materials(
    ...
    num_processes = 4  # 你希望的最大进程数量
    pool = multiprocessing.Pool(processes=num_processes)
    result_queue = multiprocessing.Manager().Queue()
    
    # 假设有多个任务要处理
    tasks = [(name, db) for name in mof_names]
    with multiprocessing.Pool(processes=num_processes) as pool:
    # 显式启动任务并设置超时
        results = [pool.apply_async(process_name, args=task) for task in tasks]
        pool.close()
        pool.join()
    """
...
if __name__ == '__main__':
    ...
    parser.add_argument('-c', '--candidates', '--candidate-file', default='./4.txt')
    parser.add_argument('-b', '--bb-dir', '--building-block-dir', default='/data/bbs')
    parser.add_argument('-t', '--topo-dir', '--topology-dir', default='/data/topologies')
    parser.add_argument('-s', '--save-dir', type=str, default='small/')
    parser.add_argument('-l', '--large-dir', type=str, default='large/')
    parser.add_argument('-co', '--cutoff', type=float, default=45.0)
