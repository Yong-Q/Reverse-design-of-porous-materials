...
def work(root_cmd, cif_dir, cif, lock, output_file, zeo_output_dir, process_bar):
    ...
    GLD,LCD, PLD, density, VSA, GSA, Vp, void_fraction = 0,0, 0, 0, 0, 0, 0, 0
    if os.system(cmd + "> /dev/null") == 0:
        try:
            with open(os.path.join(cif_dir, cif_name + '.res')) as f:
               GLD, LCD, PLD = get_LCD_PLD(f.read())
            ...
            with open(output_file, 'a') as f:
                f.write("{},{},{},{},{},{},{},{},{}\n".format(
                    cif_name,GLD, LCD, PLD, density, VSA, GSA, Vp, void_fraction))
            process_bar.incr()
            process_bar.run()
            lock.release()
...
if __name__ == "__main__":
    ...
    zeo_output_dir = "/data16t/qyPBS_WORK/zeo_results"
    ...
    with open(output_file, 'w') as f:
        f.write(
            'name,GLD,LCD,PLD,desity(g/cm^3),VSA(m^2/cm^3),GSA(m^2/g),Vp(cm^3/g),void_fraction\n')
    print("calculating.....")
    for cif in cifs:
