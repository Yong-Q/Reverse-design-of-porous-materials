...
def gather_features_main(directory, outputname):
    ...
    with open(outputname, "w", newline='') as csvfile:
        ...
        # 写入标题行
        csvwriter.writerow([
            "MOF", "H", "C", "N", "F", "Cl", "Br","I", "V", "Cu", "Zn", "Zr",
            "Metal Type", "Total Degree of Unsaturation", "Metallic Percentage",
            "Oxygen-to-Metal Ratio", "Electronegativity-to-Total Ratio",
            "Weighted Electronegativity per Atom", "Nitrogen to Oxygen"
        ])
        ...
        element_feats = ['H','C','N','F','Cl','Br','I','V','Cu','Zn','Zr']
        ...
        for cif in cif_files:
            ...
            for element in element_feats:
                if element in atom_dict:
                    new_line.append(atom_dict.get(element, 0))
                else:
                    new_line.append(0)
            
            metal_types_present = [element for element in metal_types if element in atom_dict]
            new_line.append(';'.join(metal_types_present))

            # 添加计算特征
            new_line.append(unsaturation)
            new_line.append(metalic_perct)
            new_line.append(weighted_EN)
            new_line.append(EN_ratio)
    ...
    print(f"Data has been written to {outputname}")
if __name__ == "__main__":
    directory='/home/qiuyong/zeo++-0.3/Process/MOF-Features/cif/'
    outputname='output.csv'
    gather_features_main(directory, outputname)
